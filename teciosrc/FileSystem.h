...
namespace tecplot { namespace filesystem { FILE* fileOpen(std::string const& ___1392, std::string const& ___2505); FILE* fileReopen(std::string const& ___1392, std::string const& ___2505, FILE* file); int   fileRename(std::string const& ___1392, std::string const& newFileName); int   fileRemove(std::string const& ___1392); bool  fileSize(char const* ___1392, uint64_t& sizeResult); bool  fileSize(std::string const& ___1392, uint64_t& sizeResult); std::string homeDirectory(); void convertPathDelimitersForOS(std::string&);
 #if !defined NO_THIRD_PARTY_LIBS
